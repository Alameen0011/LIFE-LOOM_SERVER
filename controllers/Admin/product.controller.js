...
export const handleAddProduct = async(req,res) => {
    try {
        ...
        const { name, description, price, category, brand, gender, sku, sizes, images } = req.body;

        const totalStock = sizes.reduce((total, size) => total + size.stock, 0);

        if(!sizes || sizes.length == 0){
            return res.status(400).json({message:"At least one size with stock is required"})
        }
...
export const handleSoftDeleteProduct = async(req,res) => {
  try {
...
export const handleFetchSingleProduct = async(req,res) => {
  try {
